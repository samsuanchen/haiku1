...
function core_words() {
  if (! window.dict || window.uniqueFCode) {
    var dict = new Object();
...
//	  paren_comment = true;		// 20151101 sam for nesting paren
  ...
  // Dump code to console.
  var ujscript=code.join('\n');
  var work1, work2, work3, work4;
  var ugo=eval(ujscript);
  window.ugo=ugo;
...
function setup3d(cv, cv3, code) {
  ...
  const fsrc = make_fragment_shader(code);
...
function draw3d(cv, cv3) {
  ...
  window.time_val_loc=time_val_loc;
...
function update_haikus(next) {
  ...
  for (var i = 0; i < haikus.length; i++) {
    ...
    if (!(haiku.style.scrolled) && code_tag)
    	 code_tag.style.width = '100%',
    	 code_tag.style.height = '100%',
    	 haiku.style.scrolled = true;
         code_tag.scrollTop = code_tag.scrollHeight; // samsuanchen@gmail.com 20180708
